...
object Function {
  ...
  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] = (t1y, t2y) =>
  // TO BE IMPLEMENTED
  for {
    t1 <- t1y
    t2 <- t2y
  ...
  def lift3[T1, T2, T3, R](f: (T1, T2, T3) => R): (Try[T1], Try[T2], Try[T3]) => Try[R] = (t1y, t2y, t3y) =>
      for {
        t1 <- t1y
        t2 <- t2y
        t3 <- t3y
  ...
  def invert3[T1, T2, T3, R](f: T1 => T2 => T3 => R): T3 => T2 => T1 => R = (t3: T3) => (t2: T2)
  => (t1: T1) => f(t1)(t2)(t3)
  // TO BE IMPLEMENTED
